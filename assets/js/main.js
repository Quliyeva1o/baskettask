...
data.products.forEach((product) => {
  ...
  const currentProd = wishlists.find((x) => x.id == product.id);
  cardsRow.innerHTML += ` <div class="col-lg-3 col-md-6 col-sm-12">
    <div class="card">
        <div class="heart" data-id=${product.id} data-name=${product.name}><i class="fa-heart ${currentProd ? 'fa-solid' : 'fa-regular'}" style="color: #ff0000;"></i></div>
        <img class="card-img-top" height="240" src="${product.imgSrc}" alt="${product.name}">
        <div class="card-body">
            <h5 class="card-title">${product.name}</h5>
            <h5>${product.hasDiscount() ? `<del class="text-secondary">${product.salePrice}$</del> ${product.calcPrice()}` : Number(product.salePrice).toFixed(2)}$</h5>
...
const wishListBtns = document.querySelectorAll('.heart');
wishListBtns.forEach((wishListBtn) => {
  wishListBtn.addEventListener('click', (e) => {
    e.preventDefault();
    const currentProdID = e.target.closest('.heart').getAttribute("data-id");
    let localWishlisArr = JSON.parse(localStorage.getItem("wishlist")) || [];
    const foundwishlist = data.products.find((x) => x.id == currentProdID);

    if (localWishlisArr.some(item => item.id === foundwishlist.id)) {
      // Remove 
      e.target.classList.replace('fa-solid', 'fa-regular');
      localWishlisArr = localWishlisArr.filter(item => item.id !== foundwishlist.id);
      localStorage.setItem("wishlist", JSON.stringify(localWishlisArr));
      console.log("Item removed from wishlist");
    } else {
      // Add
      e.target.classList.replace('fa-regular', 'fa-solid');
      localWishlisArr.push(foundwishlist);
      localStorage.setItem("wishlist", JSON.stringify(localWishlisArr));
